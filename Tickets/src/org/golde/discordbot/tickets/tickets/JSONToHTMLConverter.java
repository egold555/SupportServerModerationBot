...
public class JSONToHTMLConverter {
	...
	public static String convert(ESSBot bot, JSONTicket ticket) {
		
		StringBuilder builder = new StringBuilder();
		builder.append(getHeader(bot, ticket));
		for(JSONMessage msg : ticket.getMessages()) {
			builder.append(getMessage(msg));
		}
		builder.append(getFooter(bot, ticket));
		
		return builder.toString();
		
	}
	
	private static String getHeader(ESSBot bot, JSONTicket ticket) {
		return HEADER
				.replace("%channel_category%", "Tickets")
				.replace("%channel_name%", ticket.getChannelName())
				.replace("%guild-icon%", bot.getGuild().getIconUrl())
				;
	}
	...
	private static String getFooter(ESSBot bot, JSONTicket ticket) {
		return FOOTER
				.replace("%date%", sdf.format(new Date()).toString())
				.replace("%support_server%", bot.getJda().getSelfUser().getName())
