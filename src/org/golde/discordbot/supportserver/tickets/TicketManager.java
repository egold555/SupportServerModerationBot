...
import org.golde.discordbot.supportserver.command.support.SupportCommand;
...
public class TicketManager {
		...
		for (File file : new File("res/tickets").listFiles()) {
			if (file.isFile()) {
				try {
					...
					openTickets.add(jsonTicket.toTicket(file.getName()));
	...
	public static void create(Member member, QuestionStateAbstract state) {
		long ownerId = member.getIdLong();
		...
		ticket.create(state, onSuccess -> {
			...
			eb.setDescription("Hello " + member.getEffectiveName() + "! "
					...
					+ "**" + BaseCommand.PREFIX + "ticket close** - close your ticket\n"
					+ "**" + BaseCommand.PREFIX + "ticket add <user>** - Add a user to your ticket\n"
					+ "**" + BaseCommand.PREFIX + "ticket remove <user>** - remove a user from your ticket\n"
					+ "**" + BaseCommand.PREFIX + "ticket public** - Makes your ticket publically viewable by everyone\n"
					+ "**" + BaseCommand.PREFIX + "ticket private** - Undoes `ticket public` and makes your ticket private again\n"
	...
	public static void addMember(Member sender, TextChannel cmdchannel, Member personToAdd) {
